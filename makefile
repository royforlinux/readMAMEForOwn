...
# build rules will be included from
# src/osd/$(OSD)/$(OSD).mak
# 这个段落中似乎是可以选用win32和SDL两种OSD层的编译
# 方式，可能SDL更能支持linux等系统下的运行，不过
# 此版本中已开始使用SDL2了
#-------------------------------------------------

ifndef OSD #OSD层的标示
#前面没有定义OSD这个变量，这里就是自行检测，但是我不知道Windows_NT下是不是
#真的有OS这个系统变量，但是除了windows_NT之外就都用SDL作为OSD层了
#所以在我的系统上这里就等效成OSD=sdl 了
ifeq ($(OS),Windows_NT)
OSD = windows
TARGETOS = win32  #系统标识，话说在OSD层直接就定义真的大丈夫？
else
OSD = sdl
endif
endif

ifndef CROSS_BUILD_OSD #此标志暂时未知
CROSS_BUILD_OSD = $(OSD)
endif
...
ifeq ($(OS),Windows_NT) #OS变量不知从何而来
TARGETOS = win32 
else

ifneq ($(CROSS_BUILD),1) #同上,由名字看，可能是交叉编译标识变量

ifneq ($(OS2_SHELL),) #同上
TARGETOS = os2
else

UNAME = $(shell uname -mps) #获取系统名称,CPU架构，系统架构

#firstWord 是获取第一单词的函数，感觉这是再重复，因为过滤函数已经可以过滤掉其他字段了
...
# Autodetect PTR64 
# Google一下 PTR是指针的意思，但是这里可能主要来指向CPU构架吧
ifndef PTR64
...
# Autodetect PTR64
ifndef PTR64 #可能也是指编译环境
ifneq (,$(findstring mingw64-w64,$(PATH))) #注意，这里是测试字符串是否为空
PTR64=1
endif
endif #PTR64

endif #($(TARGETOS),win32)
...
NAME = $(TARGET)
# 在我的系统上TARGET变量最后是mame，所以名称也被赋值成mame
else
...
#第一次载入新的makefile，纪念
#调用不同的mak文件，在安装系统中只改变了CCOMFLAGS变量的值
#加了许多的警告项，从而在编译的时候我们会看到很多警告信息
ifdef SANITIZE
# 没有SANITIZE定义这个变量
CCOMFLAGS += -fsanitize=$(SANITIZE)
#在我的设置下没有给SANITIZE变量设置任何值
...
# 没有定义BUILD这个变量
# 但是定义了EMULATOR这个变量等于FULLNAME，最后等于mame也就是最后的可执行文件
...
# OBJDIRS这个变量被定义成 OBJDIRS = $(OBJ) $(OBJ)/$(TARGET)/$(SUBTARGET)
# SUBTARGET=TARGET 
# OBJ = obj/$(PREFIX)$(OSD)$(SUFFIX)$(SUFFIX64)$(SUFFIXDEBUG)$(SUFFIXPROFILE)
# 相当于OBJ=obj/sdl64
# 迄今为止主要目标已经变异完毕，下面各行不在关注
